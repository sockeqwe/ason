...
public class JsonClass {
  ...
  public JsonClass(TypeElement annotatedClass, String packageName, Config config) {
  ...
  public String getAdapterClassName() {
    return annotatedClass.getSimpleName().toString() + Ason.TYPE_ADAPTER_SUFFIX;
  }

  public void writeJavaClass(JavaWriter jw) throws IOException {

    String originClassName = annotatedClass.getSimpleName().toString();
  ...
  private void writeReadJson(String originClassName, JavaWriter jw) throws IOException {
    ...
    jw.emitEmptyLine();
    jw.emitStatement("Config config = ason.getConfig()");
    ...
    for (JsonProperty p : properties) {
      jw.beginControlFlow("while ( in.hasNext() )");

      jw.endControlFlow();
    }

    JsonReader in;
    // TODO detect inheritance etc.
  ...
  private void writeWriteJson(String originClassName, JavaWriter jw) throws IOException {
    ...
    List<String> params = Arrays.asList(
        new String[] { "JsonWriter", "out", originClassName, "value", "Ason", "ason" });

    List<String> throwing = Arrays.asList(new String[] { "IOException" });

    jw.beginMethod("void", "write", EnumSet.of(Modifier.PUBLIC), params, throwing);
